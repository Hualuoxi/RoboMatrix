	...
	for (int date = 0; date < T; ++date) {
		auto add_vir_2_ser = [&](const std::string& name, int add_id, int pos, int numa = -1) {
			...
			if (sers[pos].A.first < 0 || sers[pos].A.second < 0 ||
				...
				std::cout <<"virs: "<< virs[add_id].core << "  " << virs[add_id].cpu <<"\n";
				std::cout <<"sers: "<< sers[pos].A.first << "  " << sers[pos].A.second  <<"  "<< sers[pos].B.first<<"  "<< sers[pos].B.second <<"\n";
				exit(0);
			}
		};
		...
		for (int i = 0; i < mi_num; ++i) {
			...
			int numa = -1;
			std::cout << "vir_id:"<<vir_id <<"  pos:"<< pos <<" \n";
